...
{
	// Base case: if there is no left child (which also means no right child)
    if (!node->left)
    {
    	// Insert char and code into codes map
...
{
	// Base case: if child does not exist, add '/' to string and return
    if (!node)
    {
        s += '/';
        return;
...
{	
	...
	// Loop through input string, matching each char to a 
	// codes map value and adding the value to the encoded string
    for (int i = 0; i < S.size(); ++i)
    {
        it = codes.find(S[i]);
        encoded_string = encoded_string + it->second;
    }
}

Encoded Huffman::encode(const std::string& s)
{
	// Cast input string s to uppercase
	std::string S = s;
	std::transform(S.begin(), S.end(), S.begin(), ::toupper);
